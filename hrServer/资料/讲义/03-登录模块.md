...
element-ui没有写native，是因为开放出来了一个ckick事件

写了`native`就表示注册的是<el-button>里`原生`的<button>的点击事件

如果不写，就表示注册的是这个<el-button>按钮的点击事件
```
...
```js
 // 代理跨域的配置
    proxy: {
      ...
      // 当地址中的有 /api的时候会触发代理机制
      '/api': {
        target: 'http://ihrm-java.itheima.net/', // 跨域请求的地址  要代理的服务器地址 这里不用写 api
...
```js
const state = {
  token: null   // 设置token 为共享状态
}
```

> 我们需要知道,**`钥匙`**不能每次都通过登录获取,我们可以将token放置到本地的缓存中,本地缓存。
>
> 将token存进缓存，现在在vuex（user.js）中，当我们刷新页面时，因为当页面发生变化时，不希望每次都通过登录来获取token，需要持久化。
>
> vuex不是持久化的 => 去实现持久化
>
> `vuex + 本地缓存` 进行相应的结合
...
**处理登录的返回结构问题** `store/modules/user.js`
