...
Vue.use(Component) // 注册自己的插件 调用install
```
    ...
    return {
      ...
      rules: {
        ...
        mobile: [{ required: true, message: '手机号不能为空', trigger: 'blur' }, {
            // pattern正则表达式 如果满足 就通过校验 如果不满足不通过
          pattern: /^1[3-9]\d{9}$/, message: '手机号格式不正确', trigger: 'blur'}],
        formOfEmployment: [{ required: true, message: '聘用形式不能为空', trigger: 'blur' }],
        workNumber: [{ required: true, message: '工号不能为空', trigger: 'blur' }],
        departmentName: [{ required: true, message: '部门不能为空', trigger: 'change' }],//change
...
<el-form-item label="部门" prop="departmentName">
 ...
 @focus="getDepartments" />
        ...
        <el-tree
          ...
          default-expand-all=""
          :props="{ label: 'name' }"
          @node-click="selectNode" //节点被点击时的回调
        />
      </el-form-item>
```
...
**建立详情页路由**  `router/modules/employees.js/children下`

```js
{
    // 动态路由参数 /employees/detail/123可以访问到员工详情页面  // ?的含义表示参数可传可不传
...
export default {
  ...
  data() {
    return {
      UserComponent: 'user-info',  //动态设置组件
```
...
```js
    ...
    changeFile(file, fileList) {  // file是当前文件 fileList是当前的最新数组
      this.fileList = fileList.map(item => item)
    }

// 这里为何暂时不成功呢
      // 因为现在还没有上传 所有第二次进来的数据一定是个空的
      // 如果完成上传动作了 第一次进入 和 第二次进去的fileList的长度应该都是1 应该都有数据
      // 上传成功 数据才能进来 => 腾讯云cos
```
...
```js
    beforeUpload(file) {
      ...
      if (maxSize < file.size) {  // 超过了限制的文件大小
        this.$message.error('图片大小最大不能超过5M')
        return false
      }
      return true // return true 这样才认为可以上传
    }
```
 ...
 // 读取上半部分的内容
    async getUserDetailById() {
      ...
      if (this.userInfo.staffPhoto) {
        ...
        // 这里我们赋值，同时需要给赋值的地址一个标记 upload: true 表示已经上传成功了
...
showCodeDialog: false, // 显示二维码弹层
 ...
 <template v-slot="{row}">
              <img
                ...
                alt=""
                style="border-radius: 50%; width: 100px; height: 100px; padding: 10px"
                @click="showQrCode(row.staffPhoto)"   //点击事件
              >
            </template>
```
