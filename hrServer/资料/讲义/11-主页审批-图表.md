...
export default {
  ...
  data() {
    return {
      defaultImg: require('@/assets/common/head.jpg'),  //处理默认图片
    }
  },
  computed: {
    ...mapState(['userInfo'])  //辅助函数形式
  }
}
</script>
```
...
```vue
<img  :src="userInfo.staffPhoto" v-imagerror="defaultImg">   <!-- 处理默认图片 -->
```
...
<el-select v-model="currentMonth" size="small" style="width: 120px;marginleft:10px"@change="dateChange">
...
export default {
  props: {
    startDate: {
      type: Date,
      default: () => new Date()  // 如果没有传递startDate就取消当前时间
    }
  },
  data() {
    return {
