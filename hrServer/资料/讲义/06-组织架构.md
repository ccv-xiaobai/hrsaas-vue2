...
import { getDepartments } from '@/api/departments'  //引入接口
export default {
  components: {
    TreeTools
  },
...
> 删除之前，提示用户是否删除，然后调用删除接口    `this.$config`
...
```vue
<tree-tools slot-scope="obj" :tree-node="obj.data" @delDepts="getDepartments" /> //重新拉取数据
```
...
```js
 async getDepartments() {
      const result = await getDepartments()
      this.departs = transListToTreeData(result.depts, '')
      this.company = { name: result.companyName, manager: '负责人', id: '' }  // id : ' '
 },
```
...
//  update : props名称 ，值传给props名称的值
//  父组件 ：props名称 . sync = "data里的数据"
```
