...
export default {
  data() {
    return {
      ...
      showDialog: false,
      // 专门接收新增或者编辑的编辑的表单数据
      showPermDialog: false, // 专门用来控制显示权限弹层
      roleForm: {},
      rules: {
        name: [{ required: true, message: '角色名称不能为空', trigger: 'blur' }]
      },
      roleId: null, // 用来记录当前点击的角色id
      permData: [], // 用来存储树形结构数据
  ...
  methods: {
    ...
    // 分配权限的方法
    async  assignPerm(id) {
      // 并且要记录id => 保存 => 需要知道存到哪个id上去
      // id用来查询当前的角色到底有哪些权限
      this.roleId = id
      // 获取所有权限的数据
      // 需要将list数据转化成treeData数据
      this.permData = transListToTreeData(await getPermissionList(), '0')
      const { permIds } = await getRoleDetail(id)
      // permIds是复写的内容 => √上 权限点的复选框
      this.selectCheck = permIds // 记录当前用户权限
