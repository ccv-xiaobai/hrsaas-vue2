  ...
  methods: {
    async getPersonalDetail() {
      ...
      if (this.formData.staffPhoto) {
        // 要将数据库里读取出来的内容 进行渲染
        // 认为此时的staffPhoto 是 1,2,3,4,5 => split => [1,2,3,4,5] => [{url: 1}, {url:2}..]
        this.$refs.myStaffPhoto.fileList = this.formData.staffPhoto.split(',').map(url => ({ url }))
      }
    },
    async savePersonal() {
      const fileList = this.$refs.myStaffPhoto.fileList // 读取上传组件的内容
      // 做一个判断? 为什么嘛要做判断 要去监测是否所有的图片都已经上传完了
      // status => success 上传完毕 不是 ready 还没传完
      if (fileList.some(item => item.status !== 'success')) {
        // 表示此时肯定还有文件没有上传完成
        this.$message.warning('您当前还有文件没有上传完毕')
        return
      }
      // 保存
    ...
    async saveUser() {
    //  调用父组件
    // 将上传组件中的图片 拿出来 存到接口里面
      const fileList = this.$refs.staffPhoto.fileList // 读取上传组件的内容
      // 做一个判断? 为什么嘛要做判断 要去监测是否所有的图片都已经上传完了
      // status => success 上传完毕 不是 ready 还没传完
      if (fileList.some(item => item.status !== 'success')) {
        // 表示此时肯定还有文件没有上传完成
